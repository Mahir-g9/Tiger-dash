...
// Replace the entire initAudioAnalysis function with this updated version
function initAudioAnalysis() {
    try {
        ...
        // Removed the automatic bgm.play() call here
    } catch (e) {
        console.error("Audio analysis initialization failed:", e);
    }
}

// Replace the playerJump function with this updated version
function playerJump() {
    // Always try to start audio on first jump
    if (!bgmStarted) {
        bgm.play().then(() => {
            bgmStarted = true;
        }).catch(e => {
...
function restartGame() {
    ...
    if (!document.hidden) {
        ...
        bgm.play().catch(e => {
            console.log("Audio play blocked:", e.message);
            // Set flag to try again on next interaction
            bgmStarted = false;
        });
    }
}
