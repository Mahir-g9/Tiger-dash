...
let bgm = new Audio('bgm.mp3');
  let bgmStarted = false;  // Add this near your variable declarations
...
function draw() {
    ...
    // Draw sky with dynamic color (above and below ground)
...
function handleVisibilityChange() {
    if (document.hidden) {
        // Tab is inactive - pause BGM
        if (!bgm.paused) {
            bgm.pause();
        }
    } else {
        // Tab is active again - resume BGM if game is running
...
function handleBeforeUnload() {
    // Ensure BGM is stopped when leaving the page
    if (!bgm.paused) {
        bgm.pause();
...
function shake(shakeSpeed = 1, duration = 500) {
  // Clamp shakeSpeed to reasonable values
  ...
  // Shake distance based on speed (pixels)
  const px = Math.floor(shakeSpeed * 5);

  styleTag = document.createElement("style");
  styleTag.id = styleId;

  // Faster speed = shorter duration and bigger shake
  const animDuration = (0.6 / shakeSpeed).toFixed(2); // seconds
