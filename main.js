...
function updatePlayer() {
    ...
    // Check dangers - modified so spike doesn't kill on top
    for (let i = dangers.length - 1; i >= 0; i--) {
        function spawnDanger(x, y, width, height) {
    const danger = {
        x: x,
        y: y,
        width: width,
        ...
        color: "red",
        speed: 6,
        type: 'box' // ✅ এই লাইন যোগ করো
    };
    dangers.push(danger);
}
        if (checkCollision(player, danger)) {
            if (player.velocityY > 0 && 
                player.y + player.height <= danger.y + danger.height * 0.7) {
                // Don't trigger game over anymore; just bounce
                player.velocityY -= 20;
            }
            else if (player.velocityY < 0) {
                // Small bounce effect when hitting from below
...
function createObstacle() {
    ...
    if (type === 'fire' && Math.random() < 0.4) {
        ...
        // Create box at random height (ground or air)
        ...
        // Add dangers on top of boxes (except ground-level boxes)
...
function drawDangers() {
    dangers.forEach(danger => {
        // Draw danger (spikes) - smaller visual representation
        ctx.fillStyle = '#FF0000';
        ctx.beginPath();
        ctx.moveTo(danger.x, danger.y + danger.height);
        ctx.lineTo(danger.x + danger.width/2, danger.y);
        ctx.lineTo(danger.x + danger.width, danger.y + danger.height);
        ctx.closePath();
        ctx.fill();
        
        // Draw danger pattern (smaller to match reduced size)
        ctx.fillStyle = '#990000';
        for (let i = 0; i < 3; i++) {
            ctx.fillRect(
                danger.x + 5 + i*8, 
                danger.y + danger.height - 5, 
                5, 
                3
            );
        }
        
        // Draw warning symbol on parent box
        if (danger.parentBox) {
            ctx.fillStyle = '#FFFF00';
            ctx.beginPath();
            ctx.arc(
                danger.parentBox.x + danger.parentBox.width/2,
                danger.parentBox.y - 15,
                8,
                0,
                Math.PI * 2
            );
